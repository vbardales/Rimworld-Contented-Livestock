...
Feature: Contentment belongs to the right live animals
  ...
  Scenario: a producing colony animal receives the need at the configured starting level
    ...
    When Contented Livestock selects animal "Bessie" for visual evidence
    Then I take a screenshot "scenario 01 - producing cow has contentment"
  ...
  Scenario: producers-only adds and removes the need from a non-producer when settings close
    ...
    When Contented Livestock selects animal "Scout" for visual evidence
    Then I take a screenshot "scenario 03 - husky has contentment when producers only is off"
    When Contented Livestock sets producers-only to true and applies settings
    Then Contented Livestock animal "Scout" has no contentment need
    When Contented Livestock selects animal "Scout" for visual evidence
    Then I take a screenshot "scenario 03 - husky loses contentment when producers only is on"
    And no errors were logged
  ...
  Scenario: taming and leaving the colony update the need immediately
    ...
    When Contented Livestock selects animal "Muff" for visual evidence
    Then I take a screenshot "scenario 02 - tamed muffalo gains contentment"
    When Contented Livestock removes "Muff" from every faction
    Then Contented Livestock animal "Muff" has no contentment need
    When Contented Livestock selects animal "Muff" for visual evidence
    Then I take a screenshot "scenario 02 - factionless muffalo loses contentment"
    And no errors were logged
  ...
  Scenario: the live production factor stops below the floor and reaches the configured maximum
    ...
    Then I take a screenshot "scenario 10 - production halted below the floor"
    ...
    When Contented Livestock selects animal "Daisy" for visual evidence
    Then I take a screenshot "scenario 09 - full contentment reaches maximum rate"
    And no errors were logged
