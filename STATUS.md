...
remaining:
  - "prepared, not released (updated 2026-09-24): PUBLICATION.md section 1 holds the full Steam description in BBCode (4622 of 8000 bytes; the page created for 0.1.0 has SOURCE CODE before IF I GO QUIET, no licence line and the removed promise about saves), the dependency and content-box answers, and three thanks drafts under 1000 characters. Publication goes through the CI: this repository has no workflow yet, asked of the CI/CD session; no dry-run has run. Blocked on the tested gate, on one presentation picture still to redo (the cow with its tip: a stray tooltip, scene fixed, rerun owed) and on a yes for the tag and GitHub release. Two Pickle passes are submitted to TicketDispatcher, avec-rimmsqol and runtime-film, not yet run."
  ...
  - "verified (done -> tested), conditional scenarios: all @requires conditions (ScreenshotMode, RIMMSQOL, FilmTicks) have now run against the current revision, and there is no @wip scenario. The six RIMMSQOL scenarios passed again on 2026-09-24 under set avec-rimmsqol, 6 of 6 over four launches (feature 03 and the restart chain 05, 06, 07), on HEAD 10001d1 with Mod/ unchanged since 4531a66 and after the scenario-isolation fix (Tests/Pickle/evidence/2026-09-24/avec-rimmsqol, text files and one capture opened); this condition is met. The FilmTicks scenario passed 1/1 on 2026-09-24 under set runtime-film (see the scenario 10 line), so the three @requires conditions have all run against the current revision. The earlier FilmTicks report was labelled set=runtime-evidence and skipped under that map, which is why it was not accepted: a set name is not proof that the scenario ran with its condition present. The new one comes from set runtime-film, whose map stages FilmTicks, and its film exists."
  ...
  - "unverified (done -> tested): Execute remaining scenarios 2-5, 7-12 and 15-17 in _tools/FUNCTIONAL-SCENARIOS.md (scenario 14, adding to and removing from a save, is out of scope since 2026-09-24: no backward compatibility is handled or tested; each remaining scenario is answered with media plus a verdict from a person, see docs/MANUAL-REVIEW.md); retain Player.log plus screenshots for stable assertions and videos for timed, transitional or restart behavior, covering a new colony and an existing save."
...
choice. Those 2026-09-22 reports were replaced and deleted on 2026-09-24 by a fresh `avec-rimmsqol`
pass, 6 of 6 (`Tests/Pickle/evidence/2026-09-24/avec-rimmsqol/`); the text record of the earlier run is
`docs/runs/2026-09-22.md`. These results validate the covered UI, animal and
