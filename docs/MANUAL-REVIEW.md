...
| 15 | Settings, boundaries, reset | 8 captures | **validated**: defaults, both slider ends, plateau held at 55, eleven values kept, reset cancelled then confirmed; the buttons are clicked by translation key since `…-215f`, which passed | the reset confirmation window (large and mostly empty): design; the slider caps (stop below 50%, speed 400%): design |
